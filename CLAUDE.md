...
- **Finder paths (`localPath / linkHref / linkTarget`, `finderSettingsHtml`):** a task's files link (`links.file`) and a job's File storage link (`toolLinks.storage`) take a web link or a Mac path (`/…`, `~/…` or `file://…`). Browsers won't open file:// from a website, so on a Mac (`IS_MAC`) a path opens through the user's own macOS Shortcut named `Splice Open` (`shortcuts://run-shortcut?…&input=text`; full paths are sent as file:// URLs and the shortcut is one Run Shell Script action, `open "$1"` with input as arguments, which needs Shortcuts → Settings → Advanced → Allow Running Scripts. Open URLs does NOT work: Shortcuts can't convert the file:// text to a URL), with setup steps under Settings → Finder folders. On other devices the path button copies it (`copy-path`). Paths never go through `safeUrl`.
