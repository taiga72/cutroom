...
- **Brand guides (`brandOf / brandPanel / brandForTask / openBrand / saveBrand`, state in `BM`):** agency clients each have one, and direct-client jobs have one on the job. A task uses its client's guide, or its job's when it has no client (`brandHome`). It shows on the job page (a collapsible panel for the selected client; on "All clients", chips per client) and as a collapsible section in the task panel, with "Add do's & don'ts to checklist" (`brandToChecklist`, which skips duplicates). Saving adds a change-log entry (the user's note, or a list of what changed). PDFs upload through `storeFile()`: artifact assets, Supabase `pictures` bucket (PDFs allowed since schema update), or a data URL up to 2.5 MB locally.
