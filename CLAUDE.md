...
- **Finder paths (`localPath / linkHref / linkTarget`, `finderSettingsHtml`):** a task's files link (`links.file`) and a job's File storage link (`toolLinks.storage`) take a web link or a Mac path (`/…`, `~/…` or `file://…`). Browsers won't open file:// from a website, so on a Mac (`IS_MAC`) a path opens through the user's own macOS Shortcut named `Splice Open` (`shortcuts://run-shortcut?…&input=text`, a Run Shell Script running `open`), with setup steps under Settings → Finder folders. On other devices the path button copies it (`copy-path`). Paths never go through `safeUrl`.
