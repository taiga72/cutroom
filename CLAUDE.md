...
- **Sidebar (`renderSide`):** Overview, Archived, then a **Jobs** item that folds the job list open or closed (`S.ui.jobsOpen`, remembered). Clients aren't listed in the sidebar; they're tabs on the job page. Settings sits in the footer next to the save status. Edit buttons use `I.pencil`; `I.gear` is the Settings cog.
