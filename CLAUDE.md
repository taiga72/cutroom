...
- **Finder paths (`localPath / linkHref / linkTarget`, `finderSettingsHtml`):** a task's files link (`links.file`) and a job's File storage link (`toolLinks.storage`) take a web link or a Mac path (`/…`, `~/…` or `file://…`). Browsers won't open file:// from a website, so on a Mac (`IS_MAC`) a path opens through the user's own macOS Shortcut named `Splice Open` (`shortcuts://run-shortcut?…&input=text`; full paths are sent as file:// URLs so the shortcut is just an **Open URLs** action on Shortcut Input, with Run Shell Script `open "$1"` as the fallback), with setup steps under Settings → Finder folders. On other devices the path button copies it (`copy-path`). Paths never go through `safeUrl`.
