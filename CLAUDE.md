...
- **Work details (`workOf / workForm / workCard`):** per-job schedule, pay, location and contact, edited in the job modal ("Work details", inputs use `data-mw="path"`) and shown in the job page's **Details** tab (`detailsPane`), next to Tasks and Brand guide. Overview job cards show days, hours and the client's local time, and the Overview header totals scheduled hours per week. Time differences come from `Intl` (`tzOffset`, `tzDiff`) against this device's time zone (`MY_TZ`). A 60-second timer re-renders so the "time there" stays current. The user is paid a fixed monthly amount, so pay is information only: no invoicing or time tracking.
