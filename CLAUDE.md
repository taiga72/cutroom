...
- `jobs`: `name, type ('agency'|'direct'), color, logo, order, tools{comms,tracker,upload,storage}, toolLinks{same keys}, folder (Mac path to the job's project files), work{days[0-6], hours, shiftStart, shiftEnd (HH:MM, the user's time), pay{amount, currency, period, payday, method}, location, tz (IANA), contact{name, reach}, since, notes}`
...
- `tasks`: `jobId, clientId, clickupId (legacy, from a removed ClickUp import), name, status, priority, start, due (YYYY-MM-DD), format, length, revision, links{task,upload,file,project}, thumb (legacy: no longer shown or set; still deleted with the task), notes, checklist[{id,text,done}], created, updated, doneAt, archived, archivedAt`
- `settings/app`: `name, logo, theme ('system'|'light'|'dark'), scheme (a `SCHEMES` id, default 'splice'), textSize (0.9|1|1.12|1.25), density ('comfortable'|'compact'), dayLimit`
...
- **Look (Settings → Appearance and Accessibility, `applyLook()`):** color schemes (`SCHEMES`, Slack-style at the user's request) each have a light and a dark accent plus a colored sidebar; `schemeCss()` builds their CSS at startup and they're applied as `data-scheme` on `<html>` (the sidebar restyles its own tokens under `:root[data-scheme] .side`). Text size is the `--ts` multiplier: every px font size in the stylesheet is written `calc(Npx*var(--ts))`, so write new font sizes the same way. Compact density is `data-density="compact"`, with its rules at the very end of the stylesheet. All preview live and are saved with the account.
...
- **Narrow screens (≤820px):** the sidebar becomes a slide-in panel (`openSide / closeSide`, ☰ in `#mtop`). The top bar keeps the logo, name, search and bell; `placeTopRight()` moves `.top-right` between the header and `#mtop-slot`. The phone CSS block sits at the end of the stylesheet so it wins. Keep phone layouts compact: one-row stats, sideways-scrolling job cards, and an icon-only toolbar (view icons, a Filter button that opens the `#filter-sheet` pop-up with the priority, due and sort selects, which `openFilters / closeFilters` move in and out, and a + for New task). List rows put the status beside the name, above the due date. The Calendar fits the screen with small event bars. Empty status groups show only their header. On phones, board cards show just the logo, a one-line title, job/client and the due date.
- **Home-screen app:** `manifest.webmanifest` plus `apple-touch-icon.png` (iPhone), `icon-192.png`, `icon-512.png` and `icon-maskable-512.png` (Android), rendered from the logo SVG on white. Re-render them if the logo changes.
- **Finder paths (`localPath / linkHref / linkTarget`, `finderSettingsHtml`):** a task's **Project folder (Mac)** (`links.project`) and a job's **Project folder on your Mac** (`job.folder`, edited in the job modal, shown as the last box in the Details tab's tools) hold a Mac path (`/…`, `~/…` or `file://…`) to the user's local assets and project files. They are separate from, and in addition to, the Drive/OneDrive storage links (`links.file`, `toolLinks.storage`), which are web links to the raw footage: the user asked to keep both. `migrateStatuses()` moves any path found in the old storage fields to the new ones. Browsers won't open file:// from a website, so on a Mac (`IS_MAC`) a path opens through the user's own macOS Shortcut named `Splice Open` (`shortcuts://run-shortcut?…&input=text`; full paths are sent as file:// URLs and the shortcut is one Run Shell Script action, `open "$1"` with input as arguments, which needs Shortcuts → Settings → Advanced → Allow Running Scripts. Open URLs does NOT work: Shortcuts can't convert the file:// text to a URL), with setup steps under Settings → Finder folders. On other devices the path button copies it (`copy-path`). Paths never go through `safeUrl`.
- **Board:** one column per status in a single row (`grid-auto-flow:column`), scrolling sideways.
- **New tasks are drafts (`DRAFT`, `newTask / saveDraft / discardDraft`):** New task (toolbar button or the `n` key) opens the panel with an unsaved draft kept only in `S.tasks` on this page. `patch` and `remove` on it never write, `live()` skips it, and reloads keep it. **Save task** (needs a name) creates it for real; Cancel, Escape, closing the panel or opening another task discards it. There are no other add-task shortcuts (no per-group quick add, board "Add task" or calendar day +), at the user's request, to avoid accidental tasks.
